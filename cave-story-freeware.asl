// Cave Story (Windows freeware) autosplitter by periwinkle
...
state("Doukutsu", "1.0.0.6"){
    ...
    uint flagUCore : 0x0009DE4C;  // bit 17: hell unlocked (after defeating Undead Core)
...
init{
    ...
    if (modules.First().ModuleName != "Doukutsu.exe")
        throw new Exception("init - game module not found, retrying");
    ...
    {
        print("ERROR: Unrecognized game executable!");
        version = "";
    }
...
startup{
    ...
    settings.Add("SplitDrownLegacy", false, "Use legacy split timing (after fade-out)", "SplitDrown");
    ...
    settings.Add("SplitUndeadCoreLegacy", false, "Use legacy split timing (after fade-out) (Best Ending only)", "SplitUndeadCore");
...
split{
    ...
    if (settings["SplitDrown"]         && !vars.triggeredSplits[31] && current.mapId == 47 &&
        ((!settings["SplitDrownLegacy"] && vars.bitIsSet(current.flagCore, 0) && old.airMeter != 0 && current.airMeter == 0) ||
          (settings["SplitDrownLegacy"] && vars.bitIsSet(current.flagCore, 0) && old.fadeTimer != 37 && current.fadeTimer == 37)))                                                   { return vars.triggeredSplits[31] = true; }
    ...
    if (settings["SplitUndeadCore"]    && !vars.triggeredSplits[46] &&
        ((!settings["SplitUndeadCoreLegacy"] && current.prevMusicId == 0 && old.prevMusicId == 32 && current.gTSwaitnext == 140) ||
          (settings["SplitUndeadCoreLegacy"] && vars.bitIsSet(current.flagUCore, 17) && !vars.bitIsSet(old.flagUCore, 17))))                                                        { return vars.triggeredSplits[46] = true; }
