...
init{
    if (game.ProcessName == "Doukutsu")
    {
        ...
        if (modules.First().ModuleMemorySize == 1236992)
            version = "Humble";
        else if (modules.First().ModuleMemorySize == 1245184)
            version = "Steam";
        refreshRate = 60;
    }
    if (version == "")
        print("ERROR: Unrecognized game version!\nModuleMemorySize: " + modules.First().ModuleMemorySize);
    // Sometimes 2031616???
...
start{
    if (game.ProcessName == "Doukutsu")
        ...
        return current.prevMusicId > 0 && current.musicId == 0 && old.mapId == 72;
}
